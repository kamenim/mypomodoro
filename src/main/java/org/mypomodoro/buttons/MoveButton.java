		...
		if (selectedActivity != null) {
			if (selectedActivity.isActivity()) { // Not ToDo
				if (!selectedActivity.isDateToday()) {
					...
					String title = Labels
							.getString("ManagerListPanel.Add activity to ToDo List");
					String message = Labels
							.getString("ManagerListPanel.The date of this activity is not today. Proceed anyway?");
						...
						if (to.isMaxNbTotalEstimatedPomReached(selectedActivity)) {
							...
							message = Labels
									.getString(
											...
											org.mypomodoro.gui.ControlPanel.preferences
													.getMaxNbPomPerDay());
							reply = JOptionPane.showConfirmDialog(window,
