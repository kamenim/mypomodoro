...
public class OverestimationPanel extends JPanel implements ActivityInformation {
	...
	private void addSaveButton() {
		...
		changeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				saveOverestimation(overestimationInputFormPanel
						.getOverestimationPomodoros().getSelectedIndex() + 1);
			}
		});
	...
	public void saveOverestimation(int overestimatedPomodoros) {
		...
		if (selectedToDo != null) {
			selectedToDo.setOverestimatedPoms(selectedToDo
					.getOverestimatedPoms() + overestimatedPomodoros);
			...
			ToDoIconLabel.showIconLabel(panel.getCommentPanel().getIconLabel(),
					selectedToDo);
			ToDoIconLabel.showIconLabel(iconLabel, selectedToDo);
			if (panel.getPomodoro().getCurrentToDo().equals(selectedToDo)) {
				ToDoIconLabel.showIconLabel(panel.getUnplannedPanel()
						.getIconLabel(), selectedToDo);
			}
