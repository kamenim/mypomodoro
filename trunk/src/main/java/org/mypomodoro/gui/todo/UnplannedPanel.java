...
public class UnplannedPanel extends CreatePanel implements ActivityInformation {
	...
	protected void validActivityAction(Activity newActivity) {
		...
		if (currentToDo != null) {
			...
			Activity selectedToDo = (Activity) panel.getToDoJList()
					.getSelectedValue();
			if (selectedToDo != null) {
				if ((unplannedInputFormPanel.isSelectedInternalInterruption() || unplannedInputFormPanel
						.isSelectedExternalInterruption())) {
					ToDoIconLabel.showIconLabel(panel.getIconLabel(),
							currentToDo.equals(selectedToDo) ? selectedToDo
									: currentToDo);
					ToDoIconLabel.showIconLabel(panel.getInformationPanel()
							.getIconLabel(), selectedToDo);
					ToDoIconLabel.showIconLabel(panel.getCommentPanel()
							.getIconLabel(), selectedToDo);
					ToDoIconLabel.showIconLabel(panel.getOverestimationPanel()
							.getIconLabel(), selectedToDo);
					ToDoIconLabel.showIconLabel(panel.getIconLabel(),
							currentToDo.equals(selectedToDo) ? selectedToDo
									: currentToDo);
		...
		if (unplannedInputFormPanel.isDateToday()) {
			message = Labels
					.getString("ToDoListPanel.Unplanned activity added to ToDo List");
			...
			message = Labels
					.getString("ToDoListPanel.Unplanned activity added to Activity List");
