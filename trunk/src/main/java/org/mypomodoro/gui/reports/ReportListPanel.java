		...
		AbstractActivitiesTableModel tableModel = new AbstractActivitiesTableModel(
			...
			protected void populateData(AbstractActivities activities) {
				...
				for (int i = 0; i < activities.size() && iterator.hasNext(); i++) {
					...
					tableData[i][1] = DateUtil.getFormatedDate(currentActivity
							.getDate());
					tableData[i][2] = DateUtil.getFormatedTime(currentActivity
							.getDate());
					...
					tableData[i][7] = currentActivity.getOverestimatedPoms() > 0 ? currentActivity
							.getActualPoms()
							- currentActivity.getEstimatedPoms()
		...
		table.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					...
					public void valueChanged(ListSelectionEvent e) {
						...
						if (row > -1) {
							selectedReportId = (Integer) table.getModel()
